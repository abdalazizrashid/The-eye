...
if __name__ == "__main__":
    ...
    ap.add_argument("-p2", "--path2", required=True, help="folder to store images")
    ...
    pathToImagesFolder2 = args["path2"]
    ...
    cap2.set(cv2.CAP_PROP_FPS, framesPerSecond)


    framesPerSecond = cap.get(cv2.CAP_PROP_FPS)
    framesPerSecond = cap2.get(cv2.CAP_PROP_FPS)
    ...
    while cap.isOpened() & cap2.isOpened():
        ...
        if auto_mode:
            ...
            cv2.putText(frameCopy, str(it_secs), (width // 2, height // 2), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 200), \
                        thickness=2)
            cv2.putText(frameCopy2, str(it_secs), (width // 2, height // 2), cv2.FONT_HERSHEY_COMPLEX, 2, (0, 0, 200), \
                        thickness=2)

            if it_secs > it_secs_max:
                if not (cv2.imwrite(pathToImagesFolder + "%d" % numberOfImages + ".jpg", frame) & cv2.imwrite(pathToImagesFolder2 + "%d" % numberOfImages + ".jpg", frame2)):
                    print("Error written images to:", pathToImagesFolder + "%d" % numberOfImages + ".jpg")
                    print("Error written images to:", pathToImagesFolder2 + "%d" % numberOfImages + ".jpg")
                else:
                    print("Created Image:", pathToImagesFolder + "%d" % numberOfImages + ".jpg")
                    print("Created Image:", pathToImagesFolder2 + "%d" % numberOfImages + ".jpg")
                numberOfImages -= 1
                it_secs = 0

            # Display the resulting frame
            cv2.imshow('frameCopy', frameCopy)
            cv2.imshow('frameCopy', frameCopy2)
        else:
